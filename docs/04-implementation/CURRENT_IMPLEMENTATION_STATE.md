...
The run receipt now gives an AI agent one compact machine-readable summary of a local real-source entrypoint run, including request intent, selected scope/source refs, bounded response paths, content digests, source materialization receipt, provenance/permission/audit refs, and default-deny posture. The next useful step is to prove that an AI agent can start from that receipt alone and validate the bounded response path without direct repo file access or additional authority.
...
Local milestone verification passed for the adapter, related manifest/source-boundary contracts, baseline denial proofs, local deterministic/source-catalog proofs, repo-work guided sample, and direct adapter smoke command. PR #120 merged to `main` as `398bb62` after GitHub Actions PR run `25308566209` passed all 47 verification steps, including `Verify local real source adapter v0`.

The latest repo-first verdict confirms that the strongest next bounded implementation direction is a local real-source agent request runner v0. The local real-source adapter can now read bounded real repo context, but the next tool-shaped step is to let an AI agent provide an explicit machine-readable request artifact and receive bounded real-source response/summary/index artifacts without direct repo file access.

`feat/local-real-source-agent-request-runner-v0` adds that request-native runner.

The command is available through:

```bash
npm run tool:local-real-source-agent-request-runner-v0:run -- --request agent-context-request.repo-work-context.json --response-output local-real-source-agent-request-runner-v0.response.json --summary-output local-real-source-agent-request-runner-v0.summary.json --index-output local-real-source-agent-request-runner-v0.index.json
```
...
PR #123 merged to `main` as `1c834fd` after GitHub Actions PR run `25310897241` passed all 50 verification steps, including `Verify local real source single-command sample artifacts`.

The next bounded pass should be a repo-first verdict after the local real-source single-command sample artifact set, deciding the next agent-facing real-source usage layer without assuming runtime/MCP/API execution.

The latest repo-first verdict confirms that the strongest next bounded implementation direction is a local real-source tool-pack artifact set. The single-command real-source path now works and has sample artifacts, but an AI agent should be able to inspect one top-level package index that points to the manifest, command, sample artifacts, allowed scope/source refs, receipt/provenance/permission/audit refs, denial posture, and verifier.

`feat/local-real-source-tool-pack-artifact-set` adds that top-level package.

The writer is available through:

```bash
npm run tool:local-real-source-tool-pack:write -- --manifest-output local-json-agent-tool-manifest.json --request-output agent-context-request.real-source-tool-pack.sample.json --response-output local-real-source-tool-pack.sample.response.json --summary-output local-real-source-tool-pack.sample.summary.json --index-output local-real-source-tool-pack.sample.index.json --sample-index-output local-real-source-tool-pack.sample.artifact-set.index.json --tool-pack-index-output local-real-source-tool-pack.index.json
```
...
The tool-pack writer packages the local JSON agent manifest and the local real-source single-command sample artifacts behind one top-level index for AI-agent inspection. It proves the single-command ref, sample writer ref, selected scope/source refs, source materialization receipt refs, content digests, provenance/permission/audit refs, verifier command, and default-deny posture.

Local milestone verification passed for the real-source tool-pack, local JSON agent manifest, single-command sample artifacts, single-command agent tool, request-native real-source runner, real-source adapter, narrow read boundary, end-to-end non-executing proof, and authority-boundary denial proof. The milestone is ready for one PR from `feat/local-real-source-tool-pack-artifact-set`.

PR #124 merged to `main` as `ae1d071` after GitHub Actions PR run `25311883032` passed all 51 verification steps, including `Verify local real source tool pack artifacts`.

The latest repo-first verdict confirms that the strongest next bounded implementation direction is a local real-source tool-pack acceptance proof. The proof should start from the tool-pack index and verify the agent-usable real-source path without relying on hidden repo knowledge.

`feat/local-real-source-tool-pack-acceptance-proof` adds that proof.
...
PR #126 merged to `main` as `ab287c1` after GitHub Actions PR run `25313615248` passed all 53 verification steps, including `Verify local real source tool pack consumption CLI boundary`.

The latest repo-first verdict confirms that the strongest next bounded implementation direction is a local real-source tool-pack index consumption boundary. The explicit artifact-path consumer works, but practical AI-agent use should start from one top-level tool-pack index and one explicit consumption-summary output path.

`feat/local-real-source-tool-pack-index-consumption-boundary` adds that command.

The index consumer is available through:

```bash
npm run tool:local-real-source-tool-pack:index-consume -- --tool-pack-index local-real-source-tool-pack.index.json --consumption-summary-output local-real-source-tool-pack.index-consumption.summary.json
```
...
PR #128 merged to `main` as `b4c901d` after GitHub Actions PR run `25315098501` passed all 55 verification steps, including `Verify local real source tool pack single-command consumption v0`.

The latest repo-first verdict confirms that the strongest next bounded implementation direction is local real-source tool-pack single-command consumption sample artifacts. The single command makes the local tool practical; the next step is to materialize a complete sample artifact set for agent inspection, including the tool-pack index, consumption summary, and a run-consumption sample index.

`feat/local-real-source-tool-pack-single-command-consumption-sample` adds that sample writer.

The sample writer is available through:

```bash
npm run tool:local-real-source-tool-pack-run-consume-sample:write -- --manifest-output local-json-agent-tool-manifest.json --request-output agent-context-request.real-source-run-consume.sample.json --response-output local-real-source-run-consume.sample.response.json --summary-output local-real-source-run-consume.sample.summary.json --index-output local-real-source-run-consume.sample.index.json --sample-index-output local-real-source-run-consume.sample.artifact-set.index.json --tool-pack-index-output local-real-source-tool-pack.index.json --consumption-summary-output local-real-source-tool-pack.run-consume.sample.summary.json --run-consumption-index-output local-real-source-tool-pack.run-consume.sample.index.json
```
...
The writer runs the single-command consumption flow and writes a sample index that references the manifest, request, response, run summary, run index, sample index, tool-pack index, and consumption summary. It preserves explicit artifact paths, the allowlisted `scope:repo-work-context`, narrow real-source refs, provenance/permission/audit refs, and default-deny posture.

Local milestone verification passed for local real-source tool-pack single-command consumption sample artifacts, single-command consumption v0, index consumption boundary, real-source tool-pack, local JSON agent manifest, real-source tool-pack acceptance proof, end-to-end non-executing proof, and authority-boundary denial proof. The milestone is ready for one PR from `feat/local-real-source-tool-pack-single-command-consumption-sample`.

PR #129 merged to `main` as `f7a8f3f` after GitHub Actions PR run `25315950275` passed all 56 verification steps, including `Verify local real source tool pack single-command consumption sample artifacts`.

The latest repo-first verdict confirms that the strongest next bounded implementation direction is a local real-source agent tool readiness index. The current flow is usable, but an AI agent should have one top-level readiness artifact that points to the primary command, sample writer, sample artifacts, verifier commands, selected refs, and default-deny posture.

`feat/local-real-source-agent-tool-readiness-index` adds that readiness writer.

The readiness writer is available through:

```bash
npm run tool:local-real-source-agent-tool-readiness-index:write -- --manifest-output local-json-agent-tool-manifest.json --request-output agent-context-request.real-source-readiness.sample.json --response-output local-real-source-readiness.sample.response.json --summary-output local-real-source-readiness.sample.summary.json --index-output local-real-source-readiness.sample.index.json --sample-index-output local-real-source-readiness.sample.artifact-set.index.json --tool-pack-index-output local-real-source-tool-pack.index.json --consumption-summary-output local-real-source-readiness.consumption.summary.json --run-consumption-index-output local-real-source-readiness.run-consumption.index.json --readiness-index-output local-real-source-agent-tool.readiness.index.json
```
...
PR #131 merged to `main` as `efcc43f` after GitHub Actions PR run `25317854543` passed all 58 verification steps, including `Verify local real source agent tool readiness acceptance proof`. Direct push-run observation for merge commit `efcc43f` was not exposed through the connector in this session.

The latest repo-first verdict confirms that the strongest next bounded implementation direction is a local real-source agent tool entrypoint v0. The readiness index is valid as an AI-agent entrypoint, but the current writer still requires many individual output paths. The next practical step is one bounded command that accepts one explicit artifact directory, writes fixed artifact names under that directory, and returns the readiness index path and entrypoint summary without opening arbitrary file reads or runtime execution.

`feat/local-real-source-agent-tool-entrypoint-v0` adds that entrypoint.

The command is available through:

```bash
npm run tool:local-real-source-agent-tool-entrypoint-v0:run -- --artifact-dir local-real-source-agent-tool-artifacts
```
...
- `2026-05-04-182-local-real-source-adapter-v0-milestone-verification.md`
...
- `2026-05-04-194-state-next-step-alignment-after-local-real-source-single-command-sample-artifacts.md`
- `2026-05-04-195-repo-first-verdict-after-local-real-source-single-command-sample-artifacts.md`
- `2026-05-04-196-local-real-source-tool-pack-artifact-set.md`
- `2026-05-04-197-local-real-source-tool-pack-artifact-set-milestone-verification.md`
- `2026-05-04-198-state-next-step-alignment-after-local-real-source-tool-pack-artifact-set.md`
...
- `2026-05-04-206-state-next-step-alignment-after-local-real-source-tool-pack-consumption-cli-boundary.md`
...
- `2026-05-04-214-state-next-step-alignment-after-local-real-source-tool-pack-single-command-consumption-v0.md`
- `2026-05-04-215-repo-first-verdict-after-local-real-source-tool-pack-single-command-consumption-v0.md`
- `2026-05-04-216-local-real-source-tool-pack-single-command-consumption-sample-artifacts.md`
- `2026-05-04-217-local-real-source-tool-pack-single-command-consumption-sample-artifacts-milestone-verification.md`
- `2026-05-04-218-state-next-step-alignment-after-local-real-source-tool-pack-single-command-consumption-sample-artifacts.md`
...
- `2026-05-04-226-state-next-step-alignment-after-local-real-source-agent-tool-readiness-acceptance-proof.md`
...
- write only explicitly provided real-source tool-pack, acceptance proof, consumption summary, index-consumption summary, single-command consumption, single-command sample, readiness index artifact output paths, readiness acceptance proof temp artifact paths, and entrypoint fixed artifact paths under one explicit artifact directory;
