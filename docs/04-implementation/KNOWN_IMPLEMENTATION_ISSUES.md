...
- **Status:** open
- **Severity:** medium
- **Description:** the local JSON request/response runner shape, local JSON fixture runner proof, minimal local JSON fixture runner CLI/file boundary, first scoped local CLI file IO path, deterministic request fixture authoring helper, and single-command local wrapper create pressure to expand into generalized file loading, process orchestration, MCP/API transport behavior, provider calls, persistence adapters, model calls, or direct runtime invocation.
