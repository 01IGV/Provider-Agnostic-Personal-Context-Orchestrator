...
### 8. Risk of local JSON runner shape drifting into local execution
- **Layer / Area:** local JSON request/response runner boundary
- **Status:** open
- **Severity:** medium
- **Description:** the local JSON request/response runner shape creates pressure to add file reads/writes, CLI/process execution, fixture loading, transport behavior, or direct runtime invocation before the non-executing fixture path is fully stabilized.
...
- **Recommended next action:** keep the runner fixture-driven and shape-only until a dedicated local fixture runner proof is approved, verified, and kept default-deny.
