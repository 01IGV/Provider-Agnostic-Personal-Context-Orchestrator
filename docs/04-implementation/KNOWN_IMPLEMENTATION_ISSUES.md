...
Feature-branch verification note (April 28, 2026): local `npm run typecheck`, `npm run tool:agent-request-response-schema:verify`, and `npm run tool:agent-request-response-schema:print` passed for `feat/agent-request-response-schema-export`; the schema export remains an agent-readable contract artifact and does not add MCP/API runtime, provider calls, persistence, model calls, permission grants, or contour execution.
