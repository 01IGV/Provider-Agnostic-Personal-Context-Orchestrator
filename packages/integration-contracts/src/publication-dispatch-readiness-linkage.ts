...
export interface PublicationDispatchReadinessIntegrationWarning {
  code: string;
  message: string;
}

export interface PublicationDispatchReadinessLinkageShape<TResult = Record<string, unknown>> {
  linkage_id: string;
  ...
  linkage_boundary: {
    dispatch_ready_placeholder_only: true;
  ...
  warnings: PublicationDispatchReadinessIntegrationWarning[];
  linked_at: IsoDateTimeString;
}

export interface PublicationDispatchReadinessLinkageBuilder {
  ...
  }): PublicationDispatchReadinessLinkageShape<TResult>;
}

export const createPublicationDispatchReadinessLinkageBuilder =
  (): PublicationDispatchReadinessLinkageBuilder => {
    return {
      build<TResult = Record<string, unknown>>(input: {
        linkage_id: string;
        dispatch_readiness_id: string;
