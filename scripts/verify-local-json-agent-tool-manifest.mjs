...
const assertions = {
  ...
  manifest_lists_required_commands:
    ...
    commandRefs.includes("proof:local-real-source-agent-tool-entrypoint-acceptance:verify") &&
    commandRefs.includes("tool:local-real-source-agent-tool-run-receipt-v0:write") &&
    commandRefs.includes("proof:local-real-source-agent-tool-run-receipt-acceptance:verify") &&
    commandRefs.includes("tool:local-real-source-agent-tool-one-command-run-v0:run") &&
  ...
  manifest_exposes_local_real_source_adapter_v0:
    manifest.commands.find(
      (command) => command.command_ref === "tool:local-real-source-adapter-v0:run"
    )?.command.includes("--response-output <path> --summary-output <path> --index-output <path>") ===
    true,
  manifest_exposes_local_real_source_agent_request_runner_v0:
    manifest.commands.find(
      (command) =>
        command.command_ref === "tool:local-real-source-agent-request-runner-v0:run"
    )?.command.includes(
  ...
  manifest_exposes_local_real_source_tool_pack_writer:
    manifest.commands.find(
      (command) => command.command_ref === "tool:local-real-source-tool-pack:write"
    )?.command.includes(
      "--manifest-output <path> --request-output <path> --response-output <path> --summary-output <path> --index-output <path> --sample-index-output <path> --tool-pack-index-output <path>"
    ) === true,
  manifest_exposes_local_real_source_tool_pack_acceptance_proof:
    manifest.commands.find(
      (command) =>
  ...
  manifest_exposes_local_real_source_tool_pack_index_consumer:
    manifest.commands.find(
      (command) => command.command_ref === "tool:local-real-source-tool-pack:index-consume"
    )?.command.includes("--tool-pack-index <path> --consumption-summary-output <path>") ===
    true,
  manifest_exposes_local_real_source_tool_pack_single_command_consumption:
    manifest.commands.find(
      (command) =>
        command.command_ref === "tool:local-real-source-tool-pack:run-consume-v0"
    )?.command.includes(
  ...
  manifest_exposes_local_real_source_agent_tool_readiness_index:
    manifest.commands.find(
      (command) =>
        command.command_ref === "tool:local-real-source-agent-tool-readiness-index:write"
    )?.command.includes(
  ...
  manifest_exposes_local_real_source_agent_tool_entrypoint_acceptance_proof:
    manifest.commands.find(
      (command) =>
        command.command_ref ===
        "proof:local-real-source-agent-tool-entrypoint-acceptance:verify"
    )?.command === "npm run proof:local-real-source-agent-tool-entrypoint-acceptance:verify",
  manifest_exposes_local_real_source_agent_tool_run_receipt_v0:
    manifest.commands.find(
      (command) =>
        command.command_ref === "tool:local-real-source-agent-tool-run-receipt-v0:write"
    )?.command.includes("--entrypoint-summary <path> --receipt-output <path>") === true,
  manifest_exposes_local_real_source_agent_tool_run_receipt_acceptance_proof:
    manifest.commands.find(
      (command) =>
        command.command_ref ===
        "proof:local-real-source-agent-tool-run-receipt-acceptance:verify"
  ...
  manifest_exposes_local_real_source_agent_tool_one_command_run_v0:
    manifest.commands.find(
      (command) =>
        command.command_ref === "tool:local-real-source-agent-tool-one-command-run-v0:run"
    )?.command.includes(
      "--artifact-dir <path> [--task-signal <text>] [--read-mode <mode>] [--depth <hint>]"
    ) === true,
  ...
  manifest_local_io_policy_is_bounded:
    ...
    manifest.local_io_policy.writes_only_explicit_local_real_source_agent_tool_readiness_index_paths === true &&
    manifest.local_io_policy.writes_only_explicit_local_real_source_agent_tool_run_receipt_path === true &&
    manifest.local_io_policy.writes_only_explicit_local_real_source_agent_tool_one_command_run_artifact_dir === true &&
    manifest.local_io_policy.reads_only_run_receipt_declared_artifacts === true &&
    ...
    manifest.local_io_policy.reads_only_entrypoint_discovered_artifacts === true &&
