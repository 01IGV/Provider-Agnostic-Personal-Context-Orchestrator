...
import {
  writeLocalV0RepoWorkContextGuidedSampleArtifactSet
} from "./local-v0-repo-work-context-guided-sample-cli.mjs";

const tempDir = mkdtempSync(join(tmpdir(), "local-v0-repo-work-context-guided-sample-"));
...
const guidedRequestOutputPath = join(
  tempDir,
  "agent-context-request.repo-work-context.guided-sample.json"
);
const guidedResponseOutputPath = join(
  tempDir,
  "verified-protocol-surface-adapter.repo-work-context.guided-sample.response.json"
);
const guidedSummaryOutputPath = join(
  tempDir,
  "local-v0-repo-work-context-guided-sample.summary.json"
);
const guidedIndexOutputPath = join(
  tempDir,
  "local-v0-repo-work-context-guided-sample.index.json"
);

const readJson = (path) => JSON.parse(readFileSync(path, "utf8"));

const sampleWriteResult = writeLocalV0RepoWorkContextGuidedSampleArtifactSet({
...
const assertions = {
  repo_work_sample_artifact_set_written:
    sampleWriteResult.verification_result ===
      "local_v0_repo_work_context_guided_sample_artifact_set_written" &&
    sampleWriteResult.output_contract_ref ===
      "local-v0-repo-work-context-guided-sample-artifact-set/v1" &&
  ...
  repo_work_catalog_entry_selected:
    selectedCatalogEntry !== undefined &&
    guidedIndexArtifact.requested_sample_scope_id === selectedCatalogEntry.scope_id &&
  ...
  repo_work_response_matches_catalog:
...
const verification = {
  verification_result:
    failed.length === 0
      ? "local_v0_repo_work_context_guided_sample_artifact_set_verified"
      : "local_v0_repo_work_context_guided_sample_artifact_set_failed",
